...
def custom_imshow(fig,ax,data, data_group_grid,cmap,cbar_label=None,vmin=None,vmax=None,add_highlights=True, group_sizes = None, add_text=False, add_group_titles = True, group_title_fontsize=0.8, axislabel_size=15,cell_size=0.7, tcolor='white', add_sign_annotations=False, reference_group=None, sign_annotation_method="continuous", signs_annotation_discrete_stepsize=0.1):
    ...
    for gi,group in enumerate(data.keys()):
        ...
        highlight_builder.add_all_highlights(group_mask, bbwidth, wbwidth, merge_along_x_axis=True, merge_along_y_axis=True, add_text=add_text, tcolor=tcolor)
        # print(highlight_builder.bbox)
...
model_params = glv.model_parameters.Parameters(wordlist)
model_params.set_pattern_generator()

fn = lambda n: np.log(n)/np.log(10)

r = model_params.set_r(lgrowth=-.3, wgrowth=-.5)
M_methods = ["M1","M2","M3","M4"]
for M_method in M_methods:
    print(M_method)
    if M_method =="M1":
        M = model_params.set_M_method1()
    elif M_method =="M2":
        M = model_params.set_M_method2( ldecay = -1, wdecay = -1, ltl=-.5, wtw = -0.7, fn=fn)

    elif M_method =="M3":
        M = model_params.set_M_method3(eps=0.1, eps2=0.9)
    elif M_method =="M4":
        M = model_params.set_M_method4(ldecay = -1, wdecay = -1, ltl=-.5, wtw = -0.7, ltw=0.5, wtl=0.5,  fn=fn)
    ...
    letters = model_params.letterlist
    words = model_params.wordlist
    nodes = model_params.nodelist
    ...
    plt.hlines(len(letters)-.5,  -.5,len(nodes)-.5,   colors='black', linewidth=5)
    plt.vlines(len(letters)-.5,  -.5,len(nodes)-.5,   colors='black', linewidth=5)
    # plt.xlim([-.5,len(nodes)-.5])
    # plt.ylim([-.5,len(nodes)-.5])
    plt.xlim([-.5,len(nodes)-.5])
    plt.ylim([len(nodes)-.5,-.5])
    # plt.text(  (len(letters)-1)/2, (len(letters)-1)/2  ,  "$W_{ww}$"  , multialignment = 'center')
    # plt.text(  len(letters)-.5 + (len(words)-1)/2, (len(letters)-1)/2  ,  "$W_{wl}$" , multialignment = 'center')
    # plt.text(  (len(letters)-1)/2, len(letters)-.5 + (len(words)-1)/2  ,  "$W_{lw}$"  ,multialignment = 'center')
    # plt.text(  len(letters)-.5 + (len(words)-1)/2, len(letters)-.5 + (len(words)-1)/2  ,  "$W_{ll}$"  ,multialignment = 'center')

    # plt.scatter((len(letters)-1)/2, (len(letters)-1)/2 )
    # plt.text(  (len(letters)-1)/2, (len(letters)-1)/2  ,  "$W_{ll}$"  , ha = 'center', va = 'center', size=40, color="white", weight='bold')
    # plt.text(  (len(letters)-1)/2, (len(letters)-1)/2  ,  "$W_{ll}$"  , ha = 'center', va = 'center', size=30, weight='bold')

    # plt.text(  len(letters)-.5 + (len(words)-1)/2, (len(letters)-1)/2  ,  "$W_{wl}$" , ha = 'center', va = 'center')
    # plt.text(  (len(letters)-1)/2, len(letters)-.5 + (len(words)-1)/2  ,  "$W_{lw}$"  , ha = 'center', va = 'center')
    # plt.text(  len(letters)-.5 + (len(words)-1)/2, len(letters)-.5 + (len(words)-1)/2  ,  "$W_{ww}$"  , ha = 'center', va = 'center')
    plt.title(M_method)

    plt.subplots_adjust(top=0.8)
    plt.savefig(rf"C:\Users\mitch\Documents\OSU\Research data and code\gLoVIA-paper\example_IA_adjacency_mtx_{M_method}.png", transparent=True)

    # plt.clf()
    # plt.show()

    # fig,ax = plt.subplots()
    # # data = {M_method:w_matrix}
    # # data = {group_name: (x_labels, y_labels[::-1], data_arr_2D[::-1].T, highlights_mask[::-1].T) for group_name,(x_labels, y_labels, data_arr_2D, highlights_mask) in data.items()}
    # highlight_mask = 
    # data = {M_method:(nodes, nodes, w_matrix, h)} 
    # custom_imshow(fig,ax,data, data_group_grid,cmap,
    #             cbar_label=None,vmin=None,vmax=None,add_highlights=True, 
    #             group_sizes = None, add_text=False, add_group_titles = True, 
    #             group_title_fontsize=0.8, axislabel_size=15,cell_size=0.7, 
    #             tcolor='white', add_sign_annotations=False, 
    #             reference_group=None, sign_annotation_method="continuous", 
    #             signs_annotation_discrete_stepsize=0.1)

    # for spine in ax.spines.values():
    #     spine.set_visible(False)


    # data = (nodes, nodes[::-1], w_matrix[::-1].T, highlights_mask[::-1].T)
    ...
    pattern_mtx = model_params.create_pattern_matrix()
    colormap = mpl.colormaps['seismic']
    # fig, ax = plt.subplots()
    # ax.xaxis.tick_top()
    # plt.imshow(pattern_mtx, cmap = colormap, vmin=-1,vmax=1);plt.xticks(np.arange(len(words)),words);plt.yticks(np.arange(len(nodes)),nodes);plt.colorbar()
    # plt.title("$P$")
    # plt.subplots_adjust(top=0.8)
    # plt.savefig(rf"C:\Users\mitch\Documents\OSU\Research data and code\gLoVIA-paper\example_IA_pattern_mtx_{M_method}.png", transparent=True)
    # # plt.show()
    # plt.clf()
    ...
    plt.clf()
    plt.imshow(pattern_mtx, cmap = colormap, vmin=-1,vmax=1);plt.xticks(np.arange(len(words)),words);plt.yticks(np.arange(len(nodes)),nodes);plt.colorbar()
    plt.hlines(len(letters)-.5,  -.5,len(words)-.5,   colors='black', linewidth=5)
    # plt.vlines(len(letters)-.5,  -.5,len(nodes)-.5,   colors='black', linewidth=5)
    # plt.xlim([-.5,len(nodes)-.5])
    # plt.ylim([-.5,len(nodes)-.5])
    plt.xlim([-.5,len(words)-.5])
    plt.ylim([len(nodes)-.5,-.5])
    # plt.text(  (len(letters)-1)/2, (len(letters)-1)/2  ,  "$W_{ww}$"  , multialignment = 'center')
    # plt.text(  len(letters)-.5 + (len(words)-1)/2, (len(letters)-1)/2  ,  "$W_{wl}$" , multialignment = 'center')
    # plt.text(  (len(letters)-1)/2, len(letters)-.5 + (len(words)-1)/2  ,  "$W_{lw}$"  ,multialignment = 'center')
    ...
    edges = [( nodes[i],nodes[j],w_matrix[i,j] ) for i in range(model_params.n_nodes) for j in range(model_params.n_nodes)]
    # print(edges)
    ...
    plt.title(M_method)
    plt.subplots_adjust(top=0.8)
    plt.savefig(rf"C:\Users\mitch\Documents\OSU\Research data and code\gLoVIA-paper\example_IA_graph_{M_method}.png", transparent=True)
